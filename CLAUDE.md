...
**Business logic (`src/lib/`)**: Pure functions and integration helpers across 16 domain folders: `admin/`, `architecture-roulette/`, `blog/`, `blog-views/`, `cv-requests/`, `data-platform-maturity/`, `data-stack-roast/`, `governance-scorecard/`, `lakehouse-cost-calculator/`, `leads/`, `practitioner-tools/`, `scd-design-lab/`, `seo/`, `spark-explained/`, `sql-deathmatch/`, and `workload-placement-simulator/`. Tests are co-located as `*.test.ts`.
