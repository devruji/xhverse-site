...
**Business logic (`src/lib/`)**: Pure functions and integration helpers across 17 domain folders: `admin/`, `architecture-roulette/`, `blog/`, `blog-views/`, `cv-requests/`, `data-platform-maturity/`, `data-stack-roast/`, `egress-pricing-calculator/`, `governance-scorecard/`, `lakehouse-cost-calculator/`, `leads/`, `practitioner-tools/`, `scd-design-lab/`, `seo/`, `spark-explained/`, `sql-deathmatch/`, and `workload-placement-simulator/`. Tests are co-located as `*.test.ts`.

**Interactive tools** (`/tools/*`): Client-side compute (no server). Classic single-purpose tools keep `questions.ts` + `scoring.ts`, pricing data, or engine modules in `src/lib/<tool>/`, rendered by pages in `src/pages/tools/`. The five practitioner assessment tools share `src/lib/practitioner-tools/` and are generated by `src/pages/tools/[slug].astro`.
