...
- For Codex-published Supabase blog posts, trigger the same Pages rebuild path after the `posts` row and cover metadata have been read back, then verify production `/blog` and the article route.
